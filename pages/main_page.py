...
class OpenIisPage:
    ...
    def fill_contacts(self, test_user: User):
        with allure.step("Заполняем Фамилию"):
            self.last_name_selector.should(be.blank).type(test_user.last_name)
        with allure.step("Заполняем имя"):
            self.first_name_selector.should(be.blank).type(test_user.first_name)
        with allure.step("Заполняем Отчество"):
            self.middle_name_selector.should(be.blank).type(test_user.middle_name)
        with allure.step("Вводим номер телефона"):
            self.phone_number_selector.should(be.blank).type(test_user.phone_number)
        with allure.step("Заполняем email"):
            self.email_selector.should(be.blank).type(test_user.email)
